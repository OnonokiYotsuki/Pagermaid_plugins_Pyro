...
async def change_name_auto():
    try:
        time_cur = (
            ...
            .strftime("%Y:%m:%d:%H:%M:%S:%p:%a")
        )
        year, month, day, hour, minu, seco, p, abbwn = time_cur.split(":")
        shift = 1 if int(minu) > 30 else 0
        hsym = time_emoji_symb[(int(hour) % 12) * 2 + shift]
        year = "".join([number_emoji_symb[i] for i in year])
        month = "".join([number_emoji_symb[i] for i in month])
        day = "".join([number_emoji_symb[i] for i in day])
        hour = "".join([number_emoji_symb[i] for i in hour])
        minu = "".join([number_emoji_symb[i] for i in minu])
        _last_name = f"{year}:{month}:{day}:{hour}:{minu} {hsym}"
